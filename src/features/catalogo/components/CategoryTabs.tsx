...
function CategoryIcon({ icon: Icon }: { icon: (typeof icons)[keyof typeof icons] }) {
  return (
    <span className="grid h-[18px] w-[18px] shrink-0 place-items-center sm:size[8px]" aria-hidden="true">
      <Icon className="h-4 w-4 sm:size[10px]" strokeWidth={2} />
    </span>
  );
}
...
function CategoryLabel({ children }: { children: React.ReactNode }) {
  return (
    <span className="line-clamp-2 w-full min-w-0 text-sm [overflow-wrap:anywhere] hyphens-auto leading-[1.12] sm:leading-[1.15]">
      {children}
    </span>
  );
}
