...
function ChartCard({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return (
    <article className="rounded-sm border border-neutral-200 bg-white p-5 shadow-sm">
      <div className="mb-5">
        <h3 className="text-base font-extrabold text-central-carbon">{title}</h3>
    ...
    <section className="mb-6 grid gap-5 xl:grid-cols-2">
      <ChartCard title="Evolución de facturación" description="Facturación neta diaria dentro del período seleccionado.">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={trend} margin={{ top: 8, right: 12, left: 6, bottom: 0 }}>
            ...
            <YAxis tick={{ fontSize: 11, fill: '#77716a' }} axisLine={false} tickLine={false} tickFormatter={(value) => `$${Math.round(Number(value) / 1000)}k`} />
      ...
      <ChartCard title="Productos con mayor facturación" description="Top de productos según importe vendido, excluyendo cancelaciones.">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={products} layout="vertical" margin={{ top: 4, right: 18, left: 24, bottom: 0 }}>
            ...
            <YAxis type="category" dataKey="name" width={116} tick={{ fontSize: 10, fill: '#5f5952' }} axisLine={false} tickLine={false} />
      ...
      <ChartCard title="Ventas por categoría" description="Participación de cada categoría en la facturación de productos.">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={categories} margin={{ top: 8, right: 12, left: 6, bottom: 36 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#ece8e1" vertical={false} />
            ...
            <YAxis tick={{ fontSize: 10, fill: '#77716a' }} axisLine={false} tickLine={false} tickFormatter={(value) => `$${Math.round(Number(value) / 1000)}k`} />
            <Tooltip formatter={(value) => [formatCurrency(Number(value)), 'Facturación']} />
      ...
      <ChartCard title="Participación por método de pago" description="Distribución de facturación entre efectivo y transferencia.">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={payment} dataKey="value" nameKey="name" innerRadius={58} outerRadius={92} paddingAngle={3}>
